...
def init_db():
    ...
    # Créer admin par défaut (à changer !)
...
def generer_cle_licence():
    """Génère une clé unique format KTR-XXXX-XXXX-XXXX"""
...
def creer_licence(client_nom, telephone, type_abo, prix):
    ...
    c.execute('''INSERT INTO licences 
                 (cle, client_nom, client_telephone, type_abonnement, 
                  date_creation, date_expiration, prix_paye) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)''',
              (cle, client_nom, telephone, type_abo, 
               date_creation, expiration.isoformat(), prix))
    conn.commit()
    conn.close()
    return cle

def verifier_licence(cle):
    """Vérifie si une licence est valide et non expirée"""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
def sauvegarder_config(cle, client_final, modele, type_config, options, warp_data, config_id):
    ...
    c.execute('''INSERT INTO configurations 
                 (cle_licence, client_final, modele_mikrotik, type_config, 
                  options_json, warp_private_key, warp_public_key, warp_client_ip,
                  date_creation, config_id) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (cle, client_final, modele, type_config, 
               json.dumps(options),
               warp_data.get("private_key", ""),
               warp_data.get("public_key", ""),
               warp_data.get("client_ip", ""),
...
def get_stats():
    ...
    c.execute("SELECT * FROM licences ORDER BY id DESC LIMIT 10")
    dernieres = c.fetchall()
    conn.close()
    return {
        "total_clients": total or 0,
        "revenu_total": revenu or 0,
        "nb_configs": nb_configs,
        "dernieres_licences": dernieres
    }
