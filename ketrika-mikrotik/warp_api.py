...
"""
============================================================
KETRIKA MIKROTIK - MOTEUR ROUTEROS v7 (VERSION ULTRA-STABLE)
Compatible RouterOS 7.22+ / CHR / hAP ax / hAP ac / Lite
Mode Batch Atomique { ... } - Zero coupure Winbox
============================================================
"""
...
def generate_script(order):
    ...
    # ============================================================
    # OUVERTURE DU BLOC ATOMIQUE ROUTEROS (ACCOLADE OUVRANTE)
    # ============================================================
    p.append("{")
    p.append(':put " "')
    p.append(':put "================================================================"')
    p.append(':put "       KETRIKA MIKROTIK v7 - CONFIGURATION EN COURS...          "')
    p.append(':put "================================================================"')
    ...
    # 3. Création du Bridge LAN
    p.append(':if ([:len [/interface/bridge/find name="bridge1"]] = 0) do={')
    p.append('  :if ([:len [/interface/bridge/find name="bridge"]] > 0) do={')
    p.append('    /interface/bridge/set [find name="bridge"] name="bridge1" auto-mac=no')
    p.append('  } else={')
    p.append('    /interface/bridge/add name="bridge1" auto-mac=no comment="LAN-KETRIKA"')
    p.append('  }')
    p.append('} else={')
    p.append('  /interface/bridge/set [find name="bridge1"] auto-mac=no')
    p.append('}')
    ...
    # 7. Activation Wi-Fi Universelle (Ne plante jamais sous ROS 7.22)
    # 7.A - Wifi moderne (AX / Wifi Package)
    p.append(':do {')
    ...
    p.append('      :if ([:len [/interface/bridge/port/find interface=$n]] = 0) do={')
    p.append('        /interface/bridge/port/add bridge=bridge1 interface=$n')
    p.append('      }')
    p.append('    }')
    p.append('  }')
    p.append('} on-error={}')

    # 7.B - Wireless legacy (AC / N)
    p.append(':do {')
    ...
    p.append(':put "##                                                           ##"')
    p.append(':put "###############################################################"')
    p.append(':put "  [OK] Bridge LAN & Ports configures"')
    p.append(':put "  [OK] Passerelle IP : ' + ros_escape(gw) + '"')
    p.append(':put "  [OK] Serveur DHCP & DNS Cloudflare actifs"')
    p.append(':put "  [OK] Wi-Fi SSID : ' + ros_escape(ssid) + '"')
    p.append(':put "  [OK] Masquage TTL FAI actif (TTL=' + str(ttl) + ')"')
    if plan in ("warp", "hotspot") and warp_ip:
        p.append(':put "  [OK] WireGuard WARP : ACTIF"')
    if plan == "hotspot":
        p.append(':put "  [OK] Portail Captif Hotspot (10 tickets prets)"')
    p.append(':put "================================================================"')
    ...
    # ============================================================
    # FERMETURE DU BLOC ATOMIQUE (ACCOLADE FERMANTE)
    # ============================================================
    p.append("}")
