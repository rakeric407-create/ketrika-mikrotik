...
"""
KETRIKA MIKROTIK
Moteur de génération de scripts RouterOS v7

Version stable :

* Copie/collage WinBox sans changement de MAC pendant l'installation
* Pas de reboot automatique pendant le collage
* Configuration LAN/DHCP/Wi-Fi/WireGuard/Hotspot selon le pack
* Détection compatible /interface wifi et /interface wireless
* Nettoyage ciblé
* Pas de scheduler de reboot lancé immédiatement
  """
...
"""
Protège une valeur destinée à être placée entre guillemets
dans RouterOS.
"""
value = str(value)
value = value.replace("\", "\\")
...
"""
X25519 compatible avec la génération d'une clé publique
WireGuard.
"""
P = 2**255 - 19
...
"""
Enregistre la clé auprès du service Cloudflare utilisé
par la configuration WARP.

```
En cas d'échec, on retourne success=False.
On n'invente pas une adresse WARP comme si elle avait
été attribuée par Cloudflare.
"""
...
"""
Génère un script RouterOS v7 stable.

```
IMPORTANT :
Le script généré ne modifie PAS l'adresse MAC du WAN
pendant le collage WinBox.

Aucun reboot automatique n'est lancé.
"""
...
# Conservé pour compatibilité avec les commandes existantes.
# Il n'est volontairement PAS appliqué au script.
mac_spoof = safe_get(
    order,
    "mac_spoof",
    False
...
p.append(
    "# KETRIKA MIKROTIK - CONFIGURATION AUTOMATIQUE ROUTEROS v7"
)
...
p.append(
    ':log info "KETRIKA: Demarrage de la configuration..."'
)
...
p.append(
    ':do { /system scheduler remove [find name~"reboot"] } on-error={}'
)
...
p.append(
    ':do { /ip firewall filter remove [find] } on-error={}'
)
...
p.append(
    ':if ([:len [/ip address find interface=bridge1 address="' +
    ros_escape(gw) +
    '/24"]] = 0) do={'
)
...
p.append(
    ...
    ros_escape(gw) +
    '/24 interface=bridge1 comment="Passerelle-LAN"'
)
...
# ------------------------------------------------------------
# RouterOS legacy wireless package
# ------------------------------------------------------------
...
p.append(
    ...
    ros_escape(ssid) +
    '" frequency=auto disabled=no } on-error={}'
)
...
if plan in ("warp", "hotspot"):
    ...
    if warp_ip:
        ...
        p.append(
            '/interface wireguard add name=wg-secure mtu=1280 listen-port=0 comment="WARP-KETRIKA" private-key="' +
        ...
        p.append(
            '/ip address add address=' +
            ros_escape(warp_ip) +
            '/32 interface=wg-secure'
        )

        p.append(
            '/interface wireguard peers add interface=wg-secure public-key="' +
            CF_PUBLIC_KEY +
            '" endpoint-address=' +
            endpoint_ip +
            ' endpoint-port=' +
        ...
        if plan == "hotspot":
            ...
            p.append(
                '/ip firewall mangle add chain=prerouting in-interface=bridge1 src-address=' +
                ros_escape(net) +
                ' hotspot=auth dst-address-type=!local action=mark-routing new-routing-mark=via-secure passthrough=yes comment="Hotspot-To-WireGuard"'
            )
            ...
            p.append(
                '/ip firewall mangle add chain=prerouting in-interface=bridge1 src-address=' +
                ros_escape(net) +
                ' dst-address-type=!local action=mark-routing new-routing-mark=via-secure passthrough=yes comment="LAN-To-WireGuard"'
            )
        ...
        p.append(
            ':log warning "KETRIKA: WARP non enregistre - tunnel non active."'
        )
...
if plan == "hotspot":
    ...
    p.append(
        '/ip dns static add name=wifi.ketrika.mg address=' +
        ros_escape(gw)
    )

    p.append(
        '/ip hotspot profile add name=ketrika-hs hotspot-address=' +
        ros_escape(gw) +
        ' dns-name=wifi.ketrika.mg login-by=http-pap,cookie http-cookie-lifetime=1d use-radius=no html-directory=hotspot'
    )

    p.append(
        '/ip hotspot add name=hotspot-ketrika interface=bridge1 profile=ketrika-hs address-pool=pool-lan disabled=no'
    )
    ...
    for _ in range(10):
        ...
        p.append(
            ...
            '" password="' +
            vc +
            '" profile="1jour" comment="Ticket-KETRIKA"'
        )
...
if dl != "0" or ul != "0":
    ...
    p.append(
        '/queue simple add name="QoS-Global" target=' +
        ros_escape(net) +
        ' max-limit=' +
        lim_ul +
        '/' +
...
p.append("# ============================================================")
p.append("# KETRIKA : INSTALLATION TERMINEE")
p.append("# ============================================================")
...
p.append(
    ':put "      [+] KETRIKA - CONFIGURATION TERMINEE [+]"'
)
...
p.append(
    ':put "  -> Vous pouvez redemarrer manuellement si necessaire"'
)
...
p.append(
    "# FIN DU SCRIPT KETRIKA MIKROTIK"
)
...
4. NE redémarrez PAS le routeur pendant le collage.

5. Attendez le message :

   KETRIKA - CONFIGURATION TERMINEE
...
6. Vérifiez ensuite :
   ...
   * Wi-Fi
   * NAT
   * WireGuard si le pack l'utilise
   * Hotspot si le pack l'utilise

================================================================================
IMPORTANT
=========

Cette version ne modifie pas l'adresse MAC du WAN pendant l'installation.

Cette mesure évite notamment qu'un changement de MAC de ether1 provoque
une perte immédiate de la connexion WinBox pendant le copier-coller.

Aucun reboot automatique n'est exécuté à la fin du script.

Si un redémarrage est nécessaire, il peut être effectué manuellement après
avoir vérifié que toute la configuration est présente.
