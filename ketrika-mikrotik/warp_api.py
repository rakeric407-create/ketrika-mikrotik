#!/usr/bin/env python3
"""
KETRIKA MIKROTIK - API Cloudflare WARP et Générateur de Scripts RouterOS v7 (Version Anti-Bootloop)
"""
...
def curve25519_scalarmult(scalar):
    """Calcul de clé publique Curve25519 pure-Python"""
    P = 2**255 - 19
...
def generate_script(order):
    """Générateur de scripts RouterOS v7 sans risque de boucle de redémarrage"""
    ...
    # 1. EN-TÊTE ET NETTOYAGE
    script_parts.append(f"""# ============================================================
# KETRIKA MIKROTIK - CONFIGURATION AUTOMATIQUE ROUTEROS v7
# LICENCE : {lic}
# ============================================================

# --- Nettoyage des schedulers de sécurité ---
/system scheduler remove [find name~"reboot"]
/system scheduler remove [find name~"br-"]

# --- Nettoyage de la configuration existante ---
...
# --- Création du Bridge LAN ---
/interface bridge add name=bridge1 comment="LAN-KETRIKA"
""")

    # 2. ADJONCTION DIRECTE DES PORTS ETHERNET AU BRIDGE
    for i in range(1, info['eth_ports'] + 1):
        p = f"ether{i}"
        if p != wan:
            script_parts.append(f'/interface bridge port add bridge=bridge1 interface={p} comment="Port-LAN"')

    # 3. INTERNET & DHCP
    script_parts.append(f"""
# --- Client DHCP sur interface WAN ---
/ip dhcp-client add interface={wan} disabled=no add-default-route=yes use-peer-dns=no comment="WAN-Internet"

# --- Adresse IP Passerelle ---
/ip address add address={gw}/24 interface=bridge1 comment="Passerelle-LAN"
...
""")
    ...
    if wifi_type == 'ax':
        script_parts.append(f"""
# --- Wi-Fi 6 (AX) ---
/interface wifi set [find default-name=wifi1] configuration.ssid="{ssid}" configuration.country=madagascar security.authentication-types=wpa2-psk,wpa3-psk security.passphrase="{wifi_pass}" disabled=no
/interface bridge port add bridge=bridge1 interface=[/interface wifi get [find default-name=wifi1] name] comment="WiFi6-LAN"
""")
    elif wifi_type in ['ac', 'n']:
        script_parts.append(f"""
# --- Wi-Fi 5 / Standard (AC/N) ---
/interface wireless security-profiles set [find default=yes] mode=dynamic-keys authentication-types=wpa2-psk wpa2-pre-shared-key="{wifi_pass}"
/interface wireless set [find default-name=wlan1] mode=ap-bridge ssid="{ssid}" frequency=auto security-profile=default disabled=no
/interface bridge port add bridge=bridge1 interface=[/interface wireless get [find default-name=wlan1] name] comment="WiFi-LAN"
""")

    # 5. VPN WARP (PACKS SÉCURITÉ OU HOTSPOT)
    if plan in ['warp', 'hotspot']:
        script_parts.append(f"""
# --- Tunnel VPN WireGuard WARP ---
/interface wireguard add name=wg-secure mtu=1280 listen-port=0 private-key="{keys['private_key']}" comment="WARP-VPN"
...
# --- Masquage NAT VPN & Anti-Fuite DNS ---
...
""")
        for _ in range(10):
            ...
            script_parts.append(f'/ip hotspot user add name="{vc}" password="{vc}" profile="1jour" comment="Ticket-Auto"')

        script_parts.append("""
# --- Pare-feu Anti-Torrent ---
/ip firewall filter add chain=forward protocol=tcp dst-port=6881-6999 action=drop comment="Drop-P2P"
/ip firewall filter add chain=forward protocol=udp dst-port=6881-6999 action=drop comment="Drop-P2P"
""")

    # 7. SÉCURITÉ NAT STANDARD & MASQUAGE TTL
    script_parts.append(f"""
# --- NAT Internet Standard ---
/ip firewall nat add chain=srcnat out-interface={wan} action=masquerade
""")

    if str(ttl) != '0':
        script_parts.append(f'/ip firewall mangle add chain=postrouting action=change-ttl new-ttl=set:{ttl} passthrough=yes comment="Anti-Partage-TTL"')
    ...
    # 9. IDENTITÉ & REDÉMARRAGE SÉCURISÉ (ZÉRO RISQUE DE BOUCLE)
    script_parts.append(f"""
# --- Changement d'identité ---
/system identity set name="KETRIKA-{lic[-4:]}"

# --- Redémarrage Unique Sécurisé (Exécution unique) ---
/system scheduler add name="reboot-once" interval=0s on-event="/system scheduler remove [find name=reboot-once]; :delay 2s; /system reboot"
:delay 1s
/system scheduler set [find name=reboot-once] start-time=[/system clock get time]
""")
