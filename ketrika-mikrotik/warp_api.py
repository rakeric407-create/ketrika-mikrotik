...
"""
============================================================
KETRIKA MIKROTIK - MOTEUR ROUTEROS v7 (VERSION FINALE CERTIFIEE)
Compatible RouterOS 7.x (7.22+) / CHR / hAP ax / hAP ac / Lite
Zero blocage - Execution Atomique
============================================================
"""
...
def generate_script(order):
    ...
    # ============================================================
    # BLOC D'EXECUTION GLOBAL (Zero sous-menu, Chemins absolus /)
    # ============================================================
    p.append("{")
    p.append(':put " "')
    p.append(':put "================================================================"')
    p.append(':put "       KETRIKA MIKROTIK - CONFIGURATION EN COURS...             "')
    p.append(':put "================================================================"')

    # 1. Protection Winbox absolue
    ...
    # 3. Libération des ports et création Bridge LAN (CORRECTION auto-mac)
    p.append(':do { /interface bridge port remove [find] } on-error={}')
    p.append(':do { /interface bridge remove [find name!="bridge1"] } on-error={}')
    p.append(':if ([:len [/interface bridge find name=bridge1]] = 0) do={ /interface bridge add name=bridge1 comment="LAN-KETRIKA" }')

    # 4. Configuration WAN & MAC Spoof
    ...
    # 5. IP LAN & DHCP Server & DNS
    p.append(':do { /ip address remove [find interface=bridge1] } on-error={}')
    p.append('/ip address add address=' + ros_escape(gw) + '/24 interface=bridge1 comment="Passerelle-LAN"')
    p.append('/ip pool add name=pool-lan ranges=' + ros_escape(pool))
    p.append('/ip dhcp-server add name=dhcp-lan interface=bridge1 address-pool=pool-lan lease-time=1d disabled=no')
    p.append('/ip dhcp-server network add address=' + ros_escape(net) + ' gateway=' + ros_escape(gw) + ' dns-server=' + ros_escape(gw))
    ...
    # 6. Ports Ethernet LAN vers Bridge
    ...
    # 7. Activation Wi-Fi (AX et Wireless Legacy protégés)
    p.append(':do {')
    p.append('  :if ([:len [/interface wifi find]] > 0) do={')
    if plan == "hotspot":
        p.append('    /interface wifi set [find] configuration.mode=ap configuration.ssid="' + ros_escape(ssid) + '" security.authentication-types="" disabled=no')
    else:
        p.append('    /interface wifi set [find] configuration.mode=ap configuration.ssid="' + ros_escape(ssid) + '" security.authentication-types=wpa2-psk,wpa3-psk security.passphrase="' + ros_escape(wifi_pass) + '" disabled=no')
    ...
    if plan == "hotspot":
        ...
        p.append('    /interface wireless security-profiles set [find default=yes] mode=dynamic-keys authentication-types=wpa2-psk unicast-ciphers=aes-ccm group-ciphers=aes-ccm wpa2-pre-shared-key="' + ros_escape(wifi_pass) + '"')
    p.append('    /interface wireless set [find] mode=ap-bridge ssid="' + ros_escape(ssid) + '" disabled=no')
    ...
    # 8. WireGuard Cloudflare WARP
    if plan in ("warp", "hotspot"):
        if warp_ip:
            p.append(':do {')
            p.append('  /interface wireguard add name=wg-secure mtu=1280 listen-port=13231 comment="WARP-KETRIKA" private-key="' + ros_escape(keys["private_key"]) + '"')
    ...
    p.append(':put "##                                                           ##"')
    p.append(':put "###############################################################"')
    p.append(':put "  [OK] Bridge LAN cree et ports rattaches"')
    p.append(':put "  [OK] IP Gateway LAN : ' + ros_escape(gw) + '"')
    p.append(':put "  [OK] Serveur DHCP actif"')
    p.append(':put "  [OK] Wi-Fi SSID : ' + ros_escape(ssid) + '"')
    p.append(':put "  [OK] Anti-TTL FAI actif"')
    if plan in ("warp", "hotspot") and warp_ip:
        p.append(':put "  [OK] WireGuard WARP : ACTIF"')
    if plan == "hotspot":
        p.append(':put "  [OK] Portail Hotspot avec 10 tickets"')
    p.append(':put "================================================================"')
    p.append(':put "  >>> Configuration active immediatement sans reboot !"')
    p.append(':put "================================================================"')
    p.append(':put " "')
...
4. L'execution en bloc { ... } s'applique instantanement.
