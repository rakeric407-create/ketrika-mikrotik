# warp_api.py - KETRIKA MIKROTIK - Moteur Pure Python & Verrouillage Matériel
import secrets
import base64
import requests
import re
...
# =======================================================
# MOTEUR CRYPTO X25519 PURE PYTHON (RFC 7748)
# =======================================================
P = 2**255 - 19
A24 = 121665
...
def register_warp_account(public_key):
    try:
        res = requests.post("https://api.cloudflareclient.com/v0a2158/reg",
            json={"key": public_key, "install_id": secrets.token_hex(11),
            ...
            headers={"Content-Type": "application/json", "User-Agent": "okhttp/3.12.1",
                     "CF-Client-Version": "a-6.30-3596"}, timeout=8)
        if res.status_code in (200, 201):
            d = res.json()
            ep = d['config']['peers'][0]['endpoint']['host']
            h, p = ep.rsplit(':', 1) if ':' in ep else (ep, '2408')
            return {'success': True, 'client_ipv4': d['config']['interface']['addresses']['v4'],
                    'warp_public_key': d['config']['peers'][0]['public_key'],
                    'endpoint_host': h, 'endpoint_port': p}
    except Exception as e:
        print(f"Info API Cloudflare: {e}")
    return {'success': False}

def generate_warp_config_for_client():
    priv, pub = generate_wireguard_keys()
    info = register_warp_account(pub)
    if info.get('success'):
        return {**info, 'private_key': priv}
    return {'success': False, 'private_key': priv, 'client_ipv4': '172.16.0.2',
            'warp_public_key': 'bmXOC+F1FxEMF9dyiK2H5/1SUtzH0JuVo51h2wPfgyo=',
            'endpoint_host': 'engage.cloudflareclient.com', 'endpoint_port': '2408'}
...
:do {{
    /interface wifi set wlan1 configuration.mode=ap configuration.ssid="{s}" \\
...
"""
        if has_5ghz(m):
            cfg += f"""
:do {{
    /interface wifi set wlan2 configuration.mode=ap configuration.ssid="{s}-5G" \\
        security.authentication-types=wpa2-psk security.passphrase="{p}" \\
...
# =======================================================
# GÉNÉRATEUR DU SCRIPT VERROUILLÉ POUR 1 ROUTEUR
# =======================================================
def generate_full_script(order):
    ...
    # Attribution asynchrone des ports physiques
    bp = ""
    ...
    rl = "\n# Mode Illimite : Aucune restriction de vitesse appliquee\n" if dl == '0' else f"""
...
"""
    ...
    if needs_warp:
        ...
        warp = f"""
# =============================================
# CLOUDFLARE SECURE TUNNEL (VERROUILLE)
# =============================================
...
:delay 1s
:do {{
    /interface wireguard add name=wg-secure listen-port=13231 mtu=1420 private-key="{wc['private_key']}" comment="[KETRIKA-SECURE]"
    ...
    /ip address add address={wc['client_ipv4']}/32 interface=wg-secure comment="[KETRIKA-IP]"
    /ip firewall mangle add chain=prerouting in-interface=bridge1 dst-address=!{net} action=mark-routing new-routing-mark=via-secure passthrough=yes comment="[KETRIKA-ROUTING]"
    /ip route add dst-address=0.0.0.0/0 gateway=wg-secure routing-table=via-secure comment="[KETRIKA-ROUTE]"
    /ip firewall nat add chain=srcnat out-interface=wg-secure action=masquerade comment="[KETRIKA-NAT]"
...
"""
    ...
    if is_hs:
        ...
        hs = f"""
# =============================================
# PORTAIL HOTSPOT WIFI ZONE (1 ROUTEUR)
# =============================================
...
"""
    ...
    return f"""# =============================================
# KETRIKA MIKROTIK - SCRIPT VERROUILLE
# Pack : {order.plan_type.upper()}
# Materiel Autorise : {order.mikrotik_model}
# Cle de Licence : {order.license_key}
# Regle : 1 SEULE CLE = 1 SEUL ROUTEUR
# =============================================

:put "Verification de la licence KETRIKA..."
:delay 1s

# 1. VERROUILLAGE DANS LE SYSTEME MIKROTIK
/system note set note="KETRIKA-LICENCE-VERROUILLEE: {order.license_key} | Materiel: {order.mikrotik_model} | Client: {order.client_name}"
/system identity set name="KETRIKA-{order.order_id}"

# 2. BRIDGE PRINCIPAL
:if ([/interface bridge find name=bridge1] = "") do={{ /interface bridge add name=bridge1 protocol-mode=none comment="KETRIKA-LAN" }}

# 3. IP GATEWAY
:if ([/ip address find address="{gw}/24"] = "") do={{ /ip address add address={gw}/24 interface=bridge1 comment="[KETRIKA-GW]" }}
...
# 7. ATTRIBUTION ASYNCHRONE DES PORTS (ANTI-COUPURE)
...
:if ([/ip firewall nat find comment~"KETRIKA"] = "") do={{ /ip firewall nat add chain=srcnat out-interface={wan} action=masquerade comment="[KETRIKA-NAT]" }}
...
{warp}{hs}{rl}
:put "================================================"
:put "  CONFIGURATION TERMINEE ET VERROUILLEE !"
:put "  Licence : {order.license_key}"
:put "  Materiel : {order.mikrotik_model}"
:put "  Ce routeur est maintenant configure."
:put "================================================"
"""
