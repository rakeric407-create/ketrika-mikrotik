...
def build_raw_script(cfg):
    ...
    ip_parts = router_ip.split('.'); subnet_base = f"{ip_parts[0]}.{ip_parts[1]}.{ip_parts[2]}" if len(ip_parts) == 4 else "192.168.88"
    dhcp_pool = f"{subnet_base}.10-{subnet_base}.250"; dhcp_net = f"{subnet_base}.0/24"
    is_wifi6 = any(k in modele for k in ["ax2", "ax3", "ax 15s", "Wi-Fi 6"]); is_wireless = any(k in modele for k in ["ac2", "ac3", "lite", "Wireless"])
...
def get_config(config_id): cfg = get_config_by_id(config_id.replace('.rsc','')); return Response(build_raw_script(cfg), mimetype="text/plain") if cfg else Response("# Erreur", mimetype="text/plain")
@app.route("/download/<path:config_id>")
def download_config(config_id): cfg = get_config_by_id(config_id.replace('.rsc','')); mem = io.BytesIO(); mem.write(build_raw_script(cfg).encode('utf-8')); mem.seek(0); return send_file(mem, mimetype="text/plain", as_attachment=True, download_name="ketrika.rsc")
@app.route("/ajouter-avis", methods=["POST"])
def ajouter_avis():
    if request.form.get("nom") and request.form.get("commentaire"):
