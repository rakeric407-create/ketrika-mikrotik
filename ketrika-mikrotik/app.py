...
import sqfrfrom flask import Flask, request, Response, render_template_string, session, redirect, url_for, send_file
from database import *
from warp_api import creer_config_warp_complete
import sqlite3, secrets, string, random, io, os, traceback, html
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        .btn-copy { width: 100%; padding: 12px; border-radius: 10px; border: none; font-weight: 700; cursor: pointer; font-family: 'Space Grotesk'; text-transform: uppercase; margin-top: 8px; font-size: 12px; color: #fff; background: linear-gradient(135deg, #7c3aed, #6d28d9); }
        .btn-copy.copied { background: linear-gradient(135deg, #059669, #047857) !important; }
        ...
        .review-text { font-size: 11px; color: var(--text-body); line-height: 1.4; font-style: italic; }
        ...
        /* SÉLECTEUR BANDE PASSANTE - STYLES DE GRILLE */
        .bw-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px; margin-top: 8px; }
        .bw-card { background: #fff; border: 1px solid var(--border-light); padding: 8px 10px; border-radius: 8px; display: flex; align-items: center; gap: 8px; cursor: pointer; transition: 0.2s; }
        .bw-card.active, .bw-card:hover { border-color: var(--accent-purple); background: #f5f3ff; }
        .bw-card input[type="radio"] { width: 14px; height: 14px; accent-color: var(--accent-purple); flex-shrink: 0; }
        .bw-card-text { display: flex; flex-direction: column; min-width: 0; text-align: left; }
        .bw-card-text b { font-size: 10px; color: var(--text-dark); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .bw-card-text span { font-size: 8px; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .whatsapp-float { position: fixed; bottom: 20px; right: 20px; z-index: 9999; background: #25D366; color: #fff; padding: 10px 16px; border-radius: 30px; font-weight: 800; font-size: 12px; text-decoration: none; display: flex; align-items: center; gap: 6px; font-family: 'Space Grotesk'; box-shadow: 0 4px 15px rgba(37,211,102,0.4); }
        .wifi-box { background: #f0f9ff; border: 1px dashed #7dd3fc; padding: 12px; border-radius: 10px; margin-top: 8px; }
        .wifi-box-title { font-family: 'Space Grotesk'; font-weight: 800; font-size: 11px; color: var(--accent-cyan); margin-bottom: 6px; }
...
<body>
    ...
    <script>
        ...
        function sendAIMessage() {
            ...
            // Moteur de réponse IA intelligent
            setTimeout(() => {
...
# Modele : {modele} | Formule : {plan.upper()} | Client : {cfg.get('client', 'Client')}
...
/system identity set name="KETRIKA-{cfg.get('client', 'Client')}"
:put "==========================================================="
...
def clean_script_for_oneliner(raw_script):
    ...
    for line in raw_script.split('\n'):
        ...
        if not line or line.startswith('#'):
            continue
        lines.append(line)
    joined_cmd = " ".join(lines)
    # Protection des caractères spéciaux pour le terminal RouterOS
    escaped_cmd = joined_cmd.replace('\\', '\\\\').replace('"', '\\"')
    return escaped_cmd
...
def home():
    ...
    reviews_html = ""
    for a in liste_avis:
        nom_esc = html.escape(a[0])
        ville_esc = html.escape(a[1]) if a[1] else "MG"
        comm_esc = html.escape(a[3])
        reviews_html += f'<div class="review-card"><div class="review-header"><span class="review-name">{nom_esc} <span class="review-city">({ville_esc})</span></span><span class="stars-gold">{"⭐" * a[2]}</span></div><div class="review-text">"{comm_esc}"</div></div>'
    ...
    content = f"""
    <!-- DASHBOARD DE PERFORMANCE VISUEL EN DIRECT -->
...
def ajouter_avis():
    nom = html.escape(request.form.get("nom", "").strip())[:50]
    ville = html.escape(request.form.get("ville", "").strip())[:50]
    commentaire = html.escape(request.form.get("commentaire", "").strip())[:500]
    try:
    ...
    if nom and commentaire:
        try:
            ...
            print(f"Erreur d'ajout d'avis: {e}")
    return redirect(url_for("home"))

@app.route("/commander", methods=["POST"])
def commander():
    nom = html.escape(request.form.get("nom", "").strip())[:100]
    tel = html.escape(request.form.get("tel", "").strip())[:30]
    formule = request.form.get("formule", "basic")
    ref = html.escape(request.form.get("ref_paiement", "").strip())[:100]
    ...
    if nom and tel and ref:
        try:
            ...
            return render(f'<div class="card"><div class="alert alert-error">❌ Erreur lors de l\'enregistrement : {e}</div><a href="/" class="btn-primary">RETOUR</a></div>')
    return redirect(url_for("home"))
...
def login():
    ...
    session["licence"] = cle
    session["client"] = result.get("client", "Client")
    session["type_abo"] = result.get("type", "basic")
    return redirect(url_for("dashboard"))
...
def dashboard():
    ...
    <div class="card">
        <div class="card-title">⚙️ CONFIGURATION A à Z - {html.escape(session.get('client', 'Client'))}</div>
...
def generate():
    ...
    modele = html.escape(request.form.get("modele", ""))
    plan_key = session.get("type_abo", "basic")
    client_final = html.escape(request.form.get("client_final", "Client")).replace(" ", "_")
    ssid = html.escape(request.form.get("ssid", "KETRIKA-NET"))
    wifi_pass = html.escape(request.form.get("wifi_pass", "ketrika2025"))
    dns_name = html.escape(request.form.get("dns_name", "ketrika.wifi"))
    router_ip = html.escape(request.form.get("router_ip", "192.168.88.1"))
    bw_choice = request.form.get("bandwidth", "illimite")
    
    if bw_choice == "custom":
        bw_down = html.escape(request.form.get("custom_down", "3M"))
        bw_up = html.escape(request.form.get("custom_up", "1M"))
    else:
    ...
    if plan_key in ["warp", "hotspot", "pro"]:
        try:
            ...
            print(f"Erreur critique lors de la génération de la config Cloudflare WARP : {e}")
            warp_data = {}
    ...
    try:
        ...
        print(f"Erreur d'archivage de licence : {e}")
...
def admin_dashboard():
    ...
    rows = ""
    for cmd in cmds:
        cli_esc = html.escape(cmd[1])
        tel_esc = html.escape(cmd[2])
        ref_esc = html.escape(cmd[5])
        rows += f'<tr><td style="padding: 8px;"><b>{cli_esc}</b><br><small>{tel_esc}</small></td><td style="padding: 8px;">{cmd[3].upper()}<br><b>{cmd[4]:,} Ar</b></td><td style="padding: 8px;"><code>{ref_esc}</code></td><td style="padding: 8px;"><form method="POST" action="/admin/valider/{cmd[0]}"><button type="submit" class="btn-primary" style="padding:4px 8px; font-size:10px; margin:0;">⚡ VALIDER</button></form></td></tr>'
    
    return render(f"""
    <div class="card">
        <div class="card-title">📋 COMMANDES ({len(cmds)})</div>
        <div style="overflow-x: auto; width: 100%;">
            <table style="width:100%; border-collapse:collapse; font-size:12px; min-width: 500px;">
                <thead>
                    <tr style="border-bottom: 2px solid var(--border-light);">
                        <th style="text-align:left; padding:8px; color:var(--accent-cyan);">Client</th>
                        <th style="text-align:left; padding:8px; color:var(--accent-cyan);">Pack</th>
                        <th style="text-align:left; padding:8px; color:var(--accent-cyan);">Réf</th>
                        <th style="text-align:left; padding:8px; color:var(--accent-cyan);">Action</th>
                    </tr>
                </thead>
                <tbody>
                    {rows if rows else "<tr><td colspan=4 style=text-align:center;padding:15px;>Aucune commande</td></tr>"}
                </tbody>
            </table>
        </div>
        <a href="/admin/creer" class="btn-primary" style="margin-top:12px;">➕ CRÉER CLÉ</a>
    </div>
    """)
...
def admin_valider(cmd_id):
    ...
    if cmd:
        ...
        conn.commit()
    conn.close()
    return render(f'<div class="card"><div class="alert alert-success">✅ Validé !</div><div class="card-title">CLÉ POUR {html.escape(cmd[2]) if cmd else "Client"} :</div><div class="terminal-box">{cle}</div><a href="/admin/dashboard" class="btn-primary" style="margin-top:12px;">RETOUR</a></div>')

@app.route("/admin/creer", methods=["GET", "POST"])
def admin_creer():
    if not session.get("admin"): return redirect(url_for("admin"))
    if request.method == "POST":
        nom = html.escape(request.form.get("client", "Client"))
        tel = html.escape(request.form.get("tel", "0340000000"))
        type_abo = request.form.get("type", "basic")
        prix = TARIFS_MODULES.get(type_abo, {}).get("prix", 10000)
        
        cle = creer_licence(nom, tel, type_abo, prix)
...
# GESTIONNAIRES DE REDIRECTION EN CAS D'ERREUR
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000)om flask import Flask, request, Response, render_template_string, session, redirect, url_for, send_file
from database import *
from warp_api import creer_config_warp_complete
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        /* DASHBOARD LIVE PERFORMANCE RESTAURÉ */
        .live-dashboard {
            background: #0f172a; border: 1px solid #334155; border-radius: 14px;
            padding: 16px; margin: 12px 0; color: #fff; box-shadow: inset 0 0 20px rgba(0,0,0,0.5);
        }
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000)lite3, secrets, string, random, io, os, traceback
from datetime import datetime
