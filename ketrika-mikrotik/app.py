...
# CONFIGURATION EMAIL GMAIL AUTOMATIQUE
...
def init_all_tables():
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS commandes (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            client_nom TEXT, telephone TEXT, email TEXT, formule TEXT, montant REAL,
        ...
        function sendAIMessage() {
            ...
            setTimeout(() => {
                ...
                if(q.includes('pack') || q.includes('choisir') || q.includes('prix') || q.includes('formule')) {
                    ...
                    resp = "<b>📱 Comment payer ?</b><br>• 🇲🇬 <b>Madagascar</b>: Mvola 0382817100 / Orange Money 0373975572 (Jean Eric)<br>• 🟡 <b>Binance Pay</b>: ID 1229612637 (USDT BEP-20)<br>• 💳 <b>Carte Bancaire</b>: Contactez-nous par WhatsApp pour le lien de paiement !";
                } else if(q.includes('brancher') || q.includes('cable') || q.includes('port') || q.includes('winbox')) {
                    resp = "<b>🔌 Branchement rapide :</b><br>1. Câble Internet ➔ <b>PORT 1 (ether1)</b>.<br>2. Câble PC / Switch ➔ <b>PORTS 2 à 13</b>.<br>3. Sur Winbox, cliquez sur l'<b>Adresse MAC</b> (Neighbors) pour vous connecter !";
                } else if(q.includes('cle') || q.includes('reçu') || q.includes('sms') || q.includes('attente') || q.includes('mail')) {
                    resp = "<b>⏰ Livraison de votre clé :</b><br>Dès validation du paiement, votre clé est expédiée automatiquement par email et par SMS sous <b>15 minutes</b> max.<br>En cas d'attente, appelez le <b>038 28 171 00</b> (Jean Eric) !";
...
def home():
    ...
    <div class="card">
        ...
        <form method="POST" action="/commander">
            ...
            <label>Adresse E-mail (Recevra votre clé automatiquement) :</label>
    ...
    <div class="card">
        ...
        <div class="faq-item">
            ...
            <div class="faq-answer">Paiement Mobile Money au nom de <b>{NOM_COMPTE}</b> :<br>🟠 Orange Money : <b>{NUMERO_ORANGE}</b><br>🟡 Mvola : <b>{NUMERO_MVOLA}</b><br>Ou via <b>Binance Pay / Carte bancaire</b>.<br>Votre clé est envoyée par E-mail et SMS après validation (max 15 min).</div>
        </div>
    </div>
    """
...
def commander():
    if request.method == "POST":
        ...
        usd = TARIFS_MODULES.get(formule, {}).get("prix_usd", 2.50)
        
        contact_info = email if email else tel
        
        if nom and contact_info and ref:
            conn = sqlite3.connect(DB_FILE)
            c = conn.cursor()
            c.execute("INSERT INTO commandes (client_nom, telephone, email, formule, montant, reference_paiement, date_commande) VALUES (?, ?, ?, ?, ?, ?, ?)", (nom, tel, email, formule, montant, ref, datetime.now().strftime("%Y-%m-%d %H:%M")))
            conn.commit()
            conn.close()
            return render(f'<div class="card"><div class="alert alert-success"><b>✅ Commande enregistrée !</b></div><p style="font-size:13px; color:var(--text-body); line-height:1.6;">Merci <b>{nom}</b>.<br>Pack <b>{TARIFS_MODULES.get(formule, {}).get("nom", "Basic")}</b> ({montant:,} Ar / ${usd:.2f} USD).<br>Votre clé sera envoyée par E-mail à <b>{email}</b> sous 15 min max.<br><br>⏰ <b>En cas d'attente : Appelez le {NUMERO_MVOLA}</b></p><a href="/" class="btn-primary">RETOUR À L'ACCUEIL</a></div>')
    return redirect(url_for("home"))
...
def generate():
    ...
    # ENVOI AUTOMATIQUE DE LA CLÉ PAR EMAIL AU CLIENT DÈS GÉNÉRATION
    ...
    if cmd_data and cmd_data[0] and "@" in cmd_data[0]:
        envoyer_email_cle(cmd_data[0], cmd_data[1], cle, TARIFS_MODULES.get(cmd_data[2], {}).get("nom", "Pack KETRIKA"))
...
def admin_dashboard():
    ...
    rows = ""
    for cmd in cmds:
        rows += f'<tr><td><b>{cmd[1]}</b><br><small>{cmd[3]}</small></td><td>{cmd[4].upper()}<br><b>{cmd[5]:,} Ar</b></td><td><code>{cmd[6]}</code></td><td><form method="POST" action="/admin/valider/{cmd[0]}"><button type="submit" class="btn-primary" style="padding:4px 8px; font-size:10px; margin:0;">⚡ VALIDER &amp; MAIL</button></form></td></tr>'
    return render(f'<div class="card"><div class="card-title">📋 COMMANDES ({len(cmds)})</div><table style="width:100%; border-collapse:collapse; font-size:12px;"><tr><th style="text-align:left; padding:8px; border-bottom:1px solid var(--border-light); color:var(--accent-cyan);">Client / Email</th><th style="text-align:left; padding:8px; border-bottom:1px solid var(--border-light); color:var(--accent-cyan);">Pack</th><th style="text-align:left; padding:8px; border-bottom:1px solid var(--border-light); color:var(--accent-cyan);">Réf</th><th style="text-align:left; padding:8px; border-bottom:1px solid var(--border-light); color:var(--accent-cyan);">Action</th></tr>{rows if rows else "<tr><td colspan=4 style=text-align:center;padding:15px;>Aucune commande en attente</td></tr>"}</table><a href="/admin/creer" class="btn-primary" style="margin-top:12px;">➕ CRÉER CLÉ MANUELLE</a></div>')
...
def admin_valider(cmd_id):
    ...
    mail_status = ""
    if cmd:
        cle = creer_licence(cmd[1], cmd[3], cmd[4], cmd[5])
        c.execute("UPDATE commandes SET statut='VALIDE', cle_generee=? WHERE id=?", (cle, cmd_id))
        conn.commit()
        
        email_client = cmd[3] # Récupération de l'adresse email
        if email_client and "@" in email_client:
            ...
            if sent:
                mail_status = f"<br>📧 <b>La clé a été envoyée automatiquement à : {email_client} !</b>"
            else:
                mail_status = f"<br>⚠️ <i>E-mail non envoyé (Vérifiez les identifiants SMTP).</i>"

    conn.close()
    return render(f'<div class="card"><div class="alert alert-success">✅ Validé ! {mail_status}</div><div class="card-title">CLÉ GÉNÉRÉE POUR {cmd[1]} :</div><div class="terminal-box" id="cle_box">{cle}</div><button class="btn-copy" id="bcp" onclick="copyText(\'cle_box\',\'bcp\')">📋 COPIER LA CLÉ (POUR SMS/WHATSAPP)</button><a href="/admin/dashboard" class="btn-primary" style="margin-top:12px;">RETOUR AU DASHBOARD</a></div>')
