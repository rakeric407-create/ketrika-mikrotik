...
TARIFS_MODULES = {
    "base": {"nom": "🛡️ Pack Essentiel (Anti-Bridage)", "prix": 10000, "desc": "TTL 64 + DNS DoH + Blocage IPv6/P2P + Wi-Fi Dual Band"},
    "warp": {"nom": "🚀 Pack Blindé (Tunnel WARP VPN)", "prix": 20000, "desc": "Pack Essentiel + Chiffrement Total WireGuard + Wi-Fi Dual Band"},
    "hotspot": {"nom": "🎫 Pack Wi-Fi Zone (Hotspot + VPN)", "prix": 30000, "desc": "Pack Blindé + Tickets Hotspot + Bande Passante Contrôlée + DNS Perso"},
    "pro": {"nom": "🏢 Pack Pro WISP (PPPoE + Hotspot + VPN)", "prix": 50000, "desc": "Solution intégrale : PPPoE + Hotspot + PCQ QoS + DNS Perso"}
}
...
BANDWIDTH_PROFILES = {
    "illimite": {"nom": "⚡ ILLIMITÉ (Full Speed)", "down": "0", "up": "0", "desc": "Vitesse maximale Starlink sans restriction"},
    "ultra": {"nom": "🚀 ULTRA RAPIDE (10M/5M)", "down": "10M", "up": "5M", "desc": "10 Mbps Download / 5 Mbps Upload"},
    "rapide": {"nom": "⭐ RAPIDE (5M/2M)", "down": "5M", "up": "2M", "desc": "5 Mbps Download / 2 Mbps Upload"},
    "standard": {"nom": "📶 STANDARD (2M/1M)", "down": "2M", "up": "1M", "desc": "2 Mbps Download / 1 Mbps Upload"},
    "eco": {"nom": "🔒 ÉCONOMIQUE (1M/512K)", "down": "1M", "up": "512k", "desc": "1 Mbps Download / 512 Kbps Upload"},
    "custom": {"nom": "🎯 SUR MESURE", "down": "3M", "up": "1M", "desc": "Entrez vos propres limites personnalisées"}
}
...
<head>
    ...
    <title>KETRIKA MIKROTIK PRO • Starlink Optimizer</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes float {
            ...
            50% { transform: translateY(-25px) translateX(12px); opacity: 0.9; }
        }
        @keyframes pulse-glow {
            0%, 100% { text-shadow: 0 0 15px rgba(0, 242, 254, 0.5), 0 0 30px rgba(0, 242, 254, 0.2); }
            50% { text-shadow: 0 0 25px rgba(0, 242, 254, 0.9), 0 0 50px rgba(16, 185, 129, 0.5); }
        }
        ...
        .particle {
            position: fixed; width: 4px; height: 4px; background: var(--accent-cyan);
            border-radius: 50%; opacity: 0.3; animation: float 6s ease-in-out infinite;
            pointer-events: none; z-index: -1;
        }
        
        .container { max-width: 880px; margin: auto; position: relative; z-index: 1; animation: slide-in 0.6s ease-out; }
        
        /* TOP BAR */
        .top-nav {
            display: flex; justify-content: space-between; align-items: center;
            margin-bottom: 20px; padding: 10px 18px; background: rgba(11, 17, 32, 0.7);
        ...
        .lang-btn {
            ...
            padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 700; cursor: pointer; transition: 0.3s;
        }
        .lang-btn:hover { background: var(--accent-cyan); color: #000; }
        
        /* HEADER */
        .header { text-align: center; padding: 25px 0 35px; position: relative; }
        .header h1 { 
        ...
        .header p { color: var(--text-muted); font-size: 14px; margin-top: 8px; font-weight: 400; }
        ...
        .card:hover { transform: translateY(-3px); box-shadow: 0 25px 60px rgba(0,0,0,0.8), 0 0 30px rgba(0,242,254,0.1); }
        
        .card-title { 
            font-family: 'Space Grotesk', sans-serif; font-size: 17px; color: var(--accent-cyan); 
            margin-bottom: 18px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase;
            display: flex; align-items: center; gap: 10px;
        }

        /* INPUTS & FORMS */
        label { display: block; font-size: 11px; font-weight: 700; color: var(--text-muted); margin-top: 16px; text-transform: uppercase; letter-spacing: 1px; }
        input, select { 
        ...
        input:focus, select:focus { 
            ...
            box-shadow: 0 0 20px rgba(0, 242, 254, 0.25); background: #162032;
        }
        ...
        .btn-primary { 
            ...
            color: #030712; border: none; border-radius: 12px; 
        ...
        .btn-primary:hover { transform: translateY(-3px); box-shadow: 0 10px 30px rgba(0, 242, 254, 0.5); }
        ...
        .btn-copy {
            ...
            letter-spacing: 1px; margin-top: 10px; transition: 0.3s;
        }
        .btn-copy:hover { box-shadow: 0 8px 25px rgba(139, 92, 246, 0.4); }
        ...
        @media (max-width: 650px) { .comparison-grid { grid-template-columns: 1fr; } }
        .comp-box { padding: 18px; border-radius: 14px; border: 1px solid rgba(255,255,255,0.06); }
        .comp-box.before { background: rgba(239, 68, 68, 0.05); border-color: rgba(239, 68, 68, 0.3); }
        .comp-box.after { background: rgba(16, 185, 129, 0.05); border-color: rgba(16, 185, 129, 0.4); }
        .comp-title { font-family: 'Space Grotesk'; font-size: 15px; font-weight: 700; margin-bottom: 10px; display: flex; align-items: center; gap: 6px; }
        .comp-title.red { color: #f87171; }
        .comp-title.green { color: #34d399; }
        .comp-list { list-style: none; font-size: 13px; line-height: 2; color: var(--text-muted); }
        ...
        .plan-option:hover { border-color: var(--accent-cyan); background: #141b2f; transform: translateX(4px); }
        ...
        .bw-option { 
            background: #101524; border: 2px solid rgba(255, 255, 255, 0.05); 
            padding: 12px; border-radius: 12px; cursor: pointer; text-align: center; transition: all 0.3s ease;
        }
        .bw-option:hover { border-color: var(--accent-purple); transform: scale(1.02); }
        ...
        .bw-content b { display: block; font-size: 13px; color: #fff; margin-bottom: 3px; }
        ...
        .payment-phone { 
            ...
            color: #f59e0b; margin: 8px 0; font-weight: 800; letter-spacing: 2px; 
        ...
        .terminal-box { 
            ...
            color: var(--accent-green); padding: 18px; border-radius: 12px; 
        ...
        .badge { 
            ...
            border: 1px solid rgba(0, 242, 254, 0.3); letter-spacing: 0.5px;
        }
        .alert { padding: 14px 18px; border-radius: 12px; margin-bottom: 15px; font-size: 13px; line-height: 1.5; }
        ...
        .feature-box { background: #101524; padding: 20px; border-radius: 14px; margin-top: 14px; line-height: 2.1; font-size: 13px; border-left: 4px solid var(--accent-cyan); }
        .wifi-box { background: #101524; border: 1px dashed rgba(0, 242, 254, 0.3); padding: 20px; border-radius: 14px; margin-top: 15px; }

        /* FAQ ACCORDION */
        .faq-item { border-bottom: 1px solid rgba(255,255,255,0.05); padding: 14px 0; }
        .faq-item:last-child { border-bottom: none; }
        .faq-question { font-weight: 700; color: #fff; font-size: 14px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; }
        .faq-answer { color: var(--text-muted); font-size: 13px; line-height: 1.6; margin-top: 8px; display: none; }
        .faq-item.active .faq-answer { display: block; }
        .faq-item.active .faq-toggle { transform: rotate(180deg); color: var(--accent-cyan); }

        /* NOTIFICATION TOAST FOMO */
        #fomo-toast {
            position: fixed; bottom: 25px; left: 25px; z-index: 9999;
            background: rgba(11, 17, 32, 0.95); border: 1px solid var(--accent-green);
            padding: 14px 18px; border-radius: 14px; color: #fff; font-size: 12px;
            box-shadow: 0 10px 30px rgba(0,0,0,0.8), 0 0 20px rgba(16, 185, 129, 0.2);
            backdrop-filter: blur(15px); display: none; align-items: center; gap: 12px;
            max-width: 320px;
        }
        ...
        .whatsapp-float {
            ...
            padding: 14px 20px; border-radius: 50px; font-weight: 800; font-size: 14px;
        ...
        .whatsapp-float:hover { transform: scale(1.08) translateY(-3px); box-shadow: 0 15px 40px rgba(37, 211, 102, 0.6); }

        table { width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 13px; }
        table th, table td { padding: 12px; border-bottom: 1px solid rgba(255,255,255,0.05); text-align: left; }
        table th { color: var(--accent-cyan); font-family: 'Space Grotesk', sans-serif; font-weight: 700; }
        
        .footer { text-align: center; color: var(--text-muted); opacity: 0.8; margin: 40px 0 20px; font-size: 11px; font-weight: 300; letter-spacing: 0.5px; }
    </style>
</head>
<body>
    ...
    <a href="https://wa.me/261382817100?text=Bonjour%20KETRIKA%20MIKROTIK%2C%20je%20souhaite%20acheter%20une%20cl%C3%A9%20d%27activation%20Starlink%20!" target="_blank" class="whatsapp-float">
    ...
    <div id="fomo-toast">
        ...
        <div>
            <b id="fomo-name" style="color:var(--accent-green); display:block;">Client à Antananarivo</b>
            <span id="fomo-action" style="color:var(--text-muted);">Pack Wi-Fi Zone activé (il y a 3 min)</span>
        </div>
    </div>

    <div class="container">
        <div class="top-nav">
            <span style="font-size:12px; color:var(--text-muted);">🚀 KETRIKA OS v2.6 • Madagascar</span>
            <button class="lang-btn" onclick="toggleLang()">🇲🇬 Malagasy / 🇫🇷 Français</button>
        </div>

        <div class="header">
            <h1>⚡ KETRIKA MIKROTIK ⚡</h1>
            <p class="txt-fr">Technologie Avancée d'Optimisation Réseau & Protection Anti-Bridage Starlink</p>
            <p class="txt-mg" style="display:none;">Fitaovana matihanina manala ny bridage Starlink & manatsara ny MikroTik</p>
        </div>

        {{ content|safe }}

        <div class="footer">
            KETRIKA MIKROTIK PRO © 2026 • Ingénierie Réseau Avancée • Hotline : 038 28 171 00
        </div>
    </div>
    ...
    <script>
        ...
        // NOTIFICATIONS FOMO EN DIRECT (VILLES DE MADAGASCAR)
        const fakeSales = [
            { name: "Hery T. (Antananarivo)", action: "Pack Wi-Fi Zone 30 000 Ar activé (il y a 4 min)" },
            { name: "Boutique Rasoa (Tamatave)", action: "Débridage hAP ax2 réussi (il y a 8 min)" },
            { name: "Mamy R. (Majunga)", action: "Pack Pro WISP 50 000 Ar activé (il y a 14 min)" },
            { name: "Toky A. (Diego Suarez)", action: "Tunnel VPN WARP déployé (il y a 19 min)" },
            { name: "Faly K. (Antsirabe)", action: "Pack Wi-Fi Zone activé (il y a 27 min)" },
            { name: "Rakoto N. (Fianarantsoa)", action: "Routeur Starlink débridé (il y a 33 min)" }
        ];
        ...
        setInterval(showFomoToast, 22000);
        setTimeout(showFomoToast, 3500);

        // BANDE PASSANTE CUSTOM TOGGLE
...
def home():
    ...
    <div class="card">
        <div class="card-title">⚡ POURQUOI UTILISER KETRIKA MIKROTIK ?</div>
        <div class="comparison-grid">
            <div class="comp-box before">
                <div class="comp-title red">❌ SANS KETRIKA (STARLINK BRIDÉ)</div>
                <ul class="comp-list">
                    <li>⚠️ Détection multi-utilisateurs active</li>
                    <li>⚠️ Vitesse bridée à 2 - 5 Mbps</li>
                    <li>⚠️ Fuites IPv6 visibles par Starlink</li>
                    <li>⚠️ Saturation réseau 24h/24</li>
                </ul>
            </div>
            <div class="comp-box after">
                <div class="comp-title green">✅ AVEC KETRIKA PRO (DÉBRIDÉ)</div>
                <ul class="comp-list">
                    <li>🚀 <b>Vitesse Max Restaurée (150-250M)</b></li>
                    <li>🛡️ <b>Mangle TTL 64 (1 seul appareil vu)</b></li>
                    <li>🔒 <b>Tunnel Chiffré WireGuard WARP</b></li>
                    <li>📶 <b>Wi-Fi 5GHz Canaux 80MHz Activés</b></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <form method="POST" action="/commander">
            ...
            <div class="payment-banner">
                <b style="color:#f59e0b; font-size:13px; font-family:'Space Grotesk';">📱 VERSEMENT MOBILE MONEY</b>
                <div style="font-size:13px; color:#fff; margin-top:4px;">Envoyez la somme correspondante au numéro :</div>
            ...
            <label>Votre Numéro de Téléphone (Réception Clé par SMS) :</label>
    ...
    <!-- FAQ SECTION -->
    <div class="card">
        <div class="card-title">❓ QUESTIONS FRÉQUENTES (FAQ)</div>
        <div class="faq-item">
            <div class="faq-question"><span>Est-ce compatible avec mon antenne Starlink (Standard / Mini) ?</span> <span class="faq-toggle">▼</span></div>
            <div class="faq-answer">Oui, 100% compatible ! Il suffit de brancher le câble Starlink sur le port 1 de votre routeur MikroTik.</div>
        </div>
        <div class="faq-item">
            <div class="faq-question"><span>Combien de temps prend l'installation ?</span> <span class="faq-toggle">▼</span></div>
            <div class="faq-answer">Moins de 5 secondes chrono ! Vous collez la commande dans Winbox Terminal et tout s'applique automatiquement.</div>
        </div>
        <div class="faq-item">
            <div class="faq-question"><span>Quels sont les moyens de paiement acceptés ?</span> <span class="faq-toggle">▼</span></div>
            <div class="faq-answer">Tous les Mobile Money de Madagascar sont acceptés (Mvola, Orange Money, Airtel Money) au 038 28 171 00.</div>
        </div>
        <div class="faq-item">
            <div class="faq-question"><span>Que faire si j'ai besoin d'aide pour configurer ?</span> <span class="faq-toggle">▼</span></div>
            <div class="faq-answer">Cliquez simplement sur le bouton vert WhatsApp en bas à droite pour contacter notre support technique gratuit.</div>
        </div>
    </div>
    """
...
def commander():
    ...
    content = f"""
    <div class="card">
        <div class="alert alert-success"><b>✅ Demande de commande enregistrée avec succès !</b></div>
        <p style="font-size:14px; line-height:1.6; color:var(--text-muted);">
            Merci <b style="color:#fff;">{nom}</b>. Votre commande pour le pack <b style="color:#fff;">{TARIFS_MODULES[formule]['nom']}</b> ({montant:,} Ar) est validée.<br><br>
            Notre équipe vérifie la référence <code style="color:var(--accent-cyan);">{ref}</code> et vous expédie la clé d'activation par SMS au <b style="color:#fff;">{tel}</b> d'ici quelques minutes.
        </p>
...
def dashboard():
    ...
    feat_html = "<div>✅ Masquage TTL = 64 (Starlink)</div><div>✅ DNS Sécurisé DoH Cloudflare</div><div>✅ Blocage IPv6 & Torrents</div><div>✅ Wi-Fi Dual Band Haute Vitesse</div>"
    dns_input_html = ""
    bandwidth_html = ""
    ...
    if plan_key in ["hotspot", "pro"]:
        ...
        dns_input_html = """
        <label>🔗 Adresse DNS du Hotspot (Page de Login) :</label>
        ...
        <div class="wifi-box" style="border-color: rgba(139, 92, 246, 0.4);">
            <div style="font-size:13px; font-weight:bold; color:var(--accent-purple); margin-bottom:10px;">📊 LIMITATION BANDE PASSANTE PAR CLIENT</div>
    ...
    <div class="card">
        ...
        <form method="POST" action="/generate">
            ...
            <div class="wifi-box">
                <div style="font-size:13px; font-weight:bold; color:var(--accent-cyan); margin-bottom:8px;">📶 PARAMÈTRES WI-FI DUAL BAND (2.4 GHz + 5 GHz)</div>
            ...
            <button type="submit" class="btn-primary">🚀 GÉNÉRER L'INJECTION ({plan_info['prix']:,} Ar)</button>
        </form>
    </div>
    """
    return render(content)
...
def generate():
    ...
    bw_info = f"<br>📊 <b>Bande Passante :</b> {bw_display}" if plan_key in ["hotspot", "pro"] else ""
    ...
    <div class="card">
        ...
        <div class="alert alert-warning">
            ...
            💡 <i>Conseil : Connectez-vous sur l'adresse MAC dans Winbox pour éviter toute déconnexion !</i>
        </div>

        <div class="card-title">MÉTHODE 1 (RECOMMANDÉE) : COMMANDE UNIQUE (ANTI-DÉCONNEXION)</div>
        <p style="font-size:12px; color:var(--text-muted);">Cliquez sur le bouton ci-dessous pour copier la commande, puis collez dans <b>Winbox ➡️ New Terminal</b> :</p>
        
        <div class="terminal-box" id="cmd-oneliner">{one_liner}</div>
        <button class="btn-copy" id="btn-cp1" onclick="copyText('cmd-oneliner', 'btn-cp1')">📋 COPIER LA COMMANDE EN 1 CLIC</button>

        <hr style="border-color:rgba(255,255,255,0.06); margin:25px 0;">
        ...
        <button class="btn-copy" id="btn-cp2" onclick="copyText('cmd-online', 'btn-cp2')" style="background:#2d3748;">📋 COPIER LA COMMANDE DIRECTE</button>

        <a href="/dashboard" class="btn-primary" style="margin-top:25px;">🔄 CRÉER UNE NOUVELLE CONFIGURATION</a>
    </div>
    """
    return render(content)
...
def admin():
    ...
    return render('<div class="card"><div class="card-title">🔐 ACCÈS ADMINISTRATEUR</div><form method="POST"><input type="text" name="username" placeholder="admin" required><input type="password" name="password" placeholder="mot de passe" required><button type="submit" class="btn-primary">CONNEXION</button></form></div>')
